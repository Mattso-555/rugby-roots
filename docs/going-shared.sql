...
-- ------------------------------------------------------------
-- Base table permissions: signed-in users may reach these tables at all.
-- (Row Level Security below then decides WHO gets in. Some projects grant
-- this by default; some don't — stating it explicitly works everywhere.)
-- ------------------------------------------------------------
...
grant select, insert, update, delete on public.squad_sections to authenticated;
...
begin
  ...
  select data into team_row from squad_sections
    where squad_id = link.squad_id and section = 'team';
  ...
  return jsonb_build_object(
    ...
    'done',      done,
    'whatToBring', coalesce(team_row ->> 'whatToBring', ''),
    'bring', coalesce(team_row -> 'bring', '{}'::jsonb)
  );
end;
$$;
