...
export function printableActivities(slots, extras) {
  const fromSlot = (v) => {
    ...
    return {
      ...
      type: "Skills course",
      setup: `${c.size[0]}m × ${c.size[1]}m area${kitSummary(c) ? ` — ${kitSummary(c)}` : ""}. Set out as drawn in the app.`,
      play: c.notes || "Run the course as designed — the layout and distances are in the app.",
