...
// A plan slot is either a built-in {skill,session} or a bespoke {ownSession:id}.
// This resolves either into a uniform shape for the screens to render.
export const resolveWeek = (data, slot) => {
  if (slot && slot.ownSession) {
    const list = (data && data.customSessions) || [];
    const session = list.find((s) => s.id === slot.ownSession);
    const lib = (data && data.customActivities) || [];
    const find = (id) => lib.find((a) => a.id === id);
    const activities = session
      ? [session.warmup, session.skill, session.game].map(find).filter(Boolean)
      : [];
    return {
      own: true,
      skillId: "own",
      session: 0,
      skill: { label: session ? session.name : "My session", emoji: "\u2b50" },
      w: {
        title: session ? session.name : "My session",
