...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const base = { team:{ name:"My Squad", ageGrade:null }, players:[], plan:null, planName:null, progress:{}, attendance:{}, planB:{}, customActivities:[], customSessions:[], courses:[], weekCourses:{}, extras:{} };
  ...
  const deleteCourse = (id) =>
    setData((prev) => {
      ...
      return { ...prev, courses: (prev.courses || []).filter((c) => c.id !== id), weekCourses: wc };
    });
  ...
  const recordObservation = (playerId, { note, skillKey, level, week }) =>
    setData((prev) => ({
      ...prev,
          ...
          <>
            ...
            {tab === "sessions" && (
              <Sessions data={data} setPlan={setPlan} setSlot={setSlot} moveWeek={moveWeek} setOwnWeek={setOwnWeek} recordObservation={recordObservation} setAttendance={setAttendance} toggleAttendance={toggleAttendance}
                ...
                attachCourse={attachCourseToWeek} detachCourse={detachCourseFromWeek} />
            )}
            {tab === "players" && (
              <Players data={data}
