...
import { supabase, signOut, setPassword, getSession } from "../lib/supabaseClient.js";
...
export function CoachesCard({ syncStatus, syncDetail }) {
  ...
  useEffect(() => { getSession().then((s) => setMe(s && s.user ? s.user.email : null)); }, []);
  ...
  return (
    <Card><div className="p-4">
      ...
      {me && (
        <p className="text-xs mt-1" style={{ color: C.mute }}>
          Signed in as <b>{me}</b> — this exact address must be on the coach list.
        </p>
      )}
      <p className="text-xs mt-1" style={{ color: C.mute }}>
      ...
      <div className="flex items-center justify-between mt-4">
        <button onClick={() => syncNowManual()} aria-label="Sync now"
