...
const MOVE = {
  ...
  pass: { stroke: "#1F6F4A", dash: "1.1 0.9",  width: 0.4,  bend: 1.4 },
  ...
  return (
    <div className={compact ? "" : "mt-2"}>
      <svg viewBox={`${-PAD} ${-PAD} ${L + PAD * 2} ${W + PAD * 2}`} width="100%" role="img"
        ...
        <defs>
          {[["run", C.ink], ["pass", "#1F6F4A"], ["kick", "#B4741B"]].map(([k, col]) => (
        ...
        {moves.map((m, i) => {
          ...
          const d = curve(m.from, m.to, st.bend);
          return (
            <g key={"m" + i}>
              <path d={d} fill="none" stroke="#fff" strokeWidth={st.width + 0.36}
