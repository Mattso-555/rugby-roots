...
export default function Sessions({
  ...
  openWeek, setOpenWeek, toggleDone, setReflection, setPlanB,
  attachCourse, detachCourse, setWeekSlot,
}) {
  ...
  if (plan && openWeek) {
    ...
    return (
      <SessionDetail r={r} blockWeek={openWeek} st={st} players={data.players}
        ...
        attachCustom={attachCustom} detachCustom={detachCustom} setPlanB={setPlanB}
        attachCourse={attachCourse} detachCourse={detachCourse} setWeekSlot={setWeekSlot} />
    );
  }
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="font-bold leading-tight">
                    {r.w.title}
                    {data.weekEdits && data.weekEdits[week] && Object.keys(data.weekEdits[week]).length > 0 && (
                      <span className="ml-2 inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-bold align-middle"
                        style={{ background: C.goldSoft, color: "#8a5a00" }}>customised</span>
                    )}
                  </div>
