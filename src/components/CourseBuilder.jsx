...
const MOVE_STYLE = {
  ...
  pass: { stroke: "#1F6F4A", dash: "1.1 0.9",  bend: 1.4 },
...
export default function CourseBuilder({ start, onSave, onCancel }) {
  ...
  function up() {
    if (arrow) {
      ...
      if (len >= 3) set({ moves: [...moves, { k: arrow.k, from: arrow.from, to: arrow.to }] });
      setArrow(null);
    }
    setDrag(null);
  }
      ...
      <Card><div className="p-4">
        ...
        <div ref={scrollRef} style={zoom ? { overflowX: "auto", WebkitOverflowScrolling: "touch" } : undefined}>
          <svg ref={svgRef}
            ...
            <defs>
              {[["run", "#12241C"], ["pass", "#1F6F4A"], ["kick", "#B4741B"]].map(([k, col]) => (
        ...
        <div className="flex items-center justify-between mt-1.5">
          <span className="text-xs" style={{ color: C.mute }}>
            {items.length} piece{items.length === 1 ? "" : "s"} of kit · {moves.length} arrow{moves.length === 1 ? "" : "s"} · {L}m × {W}m
          </span>
          {(items.length > 0 || moves.length > 0 || labels.length > 0) && (
            <button type="button"
              onClick={() => { if (window.confirm("Clear the whole course?")) set({ items: [], moves: [], labels: [] }); }}
