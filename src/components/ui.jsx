...
export function Label({ children, className = "" }) {
  // A short accent rule before the label reads as intentional structure,
  // not a bare grey caption — a small thing that lifts every card.
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <span aria-hidden style={{ width: 14, height: 2, borderRadius: 2, background: C.gold, flex: "0 0 auto" }} />
      <span className="text-xs font-bold uppercase" style={{ color: C.mute, letterSpacing: ".07em" }}>{children}</span>
    </div>
  );
}
export function SectionTitle({ children }) {
  return <h1 className="font-display font-extrabold" style={{ color: C.pine, fontSize: 26, letterSpacing: "-.03em", lineHeight: 1.1 }}>{children}</h1>;
}
export function MiniStat({ value, label, gold }) {
  return <Card style={gold ? { background: C.goldSoft, borderColor: C.goldSoft } : {}}>
    <div className="p-3 text-center">
      <div className="font-display text-3xl font-extrabold" style={{ color: gold ? "#8a5a00" : C.pine, fontVariantNumeric: "tabular-nums", letterSpacing: "-.02em" }}>{value}</div>
...
export function Icon({ name, size = 22 }) {
  const paths = {
    ...
    library: <><path d="M4 5.5A1.5 1.5 0 0 1 5.5 4H10v16H5.5A1.5 1.5 0 0 1 4 18.5z" /><path d="M14 4h4.5A1.5 1.5 0 0 1 20 5.5v13a1.5 1.5 0 0 1-1.5 1.5H14z" /><path d="M14 8h6" /></>,
  };
  return (
