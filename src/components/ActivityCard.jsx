...
import { organise, pitchFor, onPitch } from "../lib/grouping.js";

export default function ActivityCard({ a, count }) {
  const group = count ? organise(count, a.type) : null;
  const pitch = count ? pitchFor(onPitch(count, a.type)) : null;
  ...
  return (
    <Card style={isGame ? { borderColor: C.gold, borderWidth: 2 } : undefined}>
      ...
      {a.diagram && <Diagram diagram={a.diagram} pitch={pitch} />}

      {group && (
        <div className="rounded-xl p-2.5 mt-3" style={{ background: C.grassSoft }}>
          <Label>With {count} tonight</Label>
          <div className="text-sm font-bold mt-0.5" style={{ color: C.pine }}>{group.headline}</div>
          <div className="text-xs mt-0.5" style={{ color: C.mute }}>{group.detail}</div>
        </div>
      )}
      ...
      {a.condition && (
        <div className="mt-3 rounded-xl p-2.5" style={{ background: C.goldSoft, border: `1px solid ${C.gold}` }}>
          ...
          <p className="text-xs mt-1" style={{ color: "#7a5d22" }}>
