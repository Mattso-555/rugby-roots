...
import { planFor, resolveSlot, buildSingle, presetsForAge, skillsForAge } from "../lib/helpers.js";
import { Card, Label, SectionTitle, Pill } from "./ui.jsx";

export default function BlockBuilder({ data, ageGrade, setPlan, setSlot, moveWeek, onDone }) {
                  ...
                  <div className="p-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col gap-1 shrink-0">
                        <button onClick={() => moveWeek(week, -1)} disabled={week === 1}
                          ...
                          className="rounded-lg w-7 h-7 flex items-center justify-center font-bold"
                          style={{ background: week === 1 ? C.paper : C.grassSoft,
                                   color: week === 1 ? C.line : C.pine, fontSize: 13 }}>▲</button>
                        <button onClick={() => moveWeek(week, 1)} disabled={week === plan.length}
                          aria-label="Move later"
                          className="rounded-lg w-7 h-7 flex items-center justify-center font-bold"
                          style={{ background: week === plan.length ? C.paper : C.grassSoft,
                                   color: week === plan.length ? C.line : C.pine, fontSize: 13 }}>▼</button>
                      </div>

                      <div className="font-display rounded-xl w-9 h-9 flex items-center justify-center font-extrabold shrink-0"
                    ...
                    {!open ? (
                      ...
                      <div className="mt-3 rounded-xl p-3" style={{ background: C.paper, border: `1px solid ${C.line}` }}>
                        <Label>Pick a skill for week {week}</Label>
                        ...
                        <Label className="mt-3">Which session?</Label>
